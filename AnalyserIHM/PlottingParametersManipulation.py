...
class PlottingParametersManipulation(QToolBox):
    def __init__(self, plotting_board_reference=None):
        ...
        self.iso_siso_display_config_button.setStyleSheet(
            "QPushButton{border-image: url(:/iso_siso_display_config.png);}")
        ...
        self.iso_siso_information_export_button.setStyleSheet(
            "QPushButton{border-image: url(:/import_cutting_information.png);}")
        ...
        self.connect(self.ordinateManipulationTree, SIGNAL("itemChanged(QTreeWidgetItem*,int)"),
                     self.handle_item_checked)
        self.connect(self.ordinateManipulationTree, SIGNAL("customContextMenuRequested(QPoint)"),
                     self.ordinate_manipulation_tree_widget_custom_context_menu_requested)
    ...
    def ordinate_manipulation_tree_widget_custom_context_menu_requested(self, pos):
        ...
        self.index = self.ordinateManipulationTree.indexOfTopLevelItem(
            self.ordinateManipulationTree.itemAt(pos).parent())
    ...
    def delete_ordinate(self):
        ...
        if self.plottingBoardReference.delete_the_ordinate_on_the_canvas(ordinate_name, filename):
            ...
            if parent.childCount() == 0:
                self.ordinateManipulationTree.takeTopLevelItem(
                    self.ordinateManipulationTree.indexOfTopLevelItem(parent))
        ...
        self.analyserMediator.generate_an_action_into_global_context(
            'remove the ordinate: ' + ordinate_name + ' in file: ' + filename + ' at position ' + str(self.index)
            + ' for plotting board :'
    ...
    def delete_the_label(self):
        ...
        self.analyserMediator.generate_an_action_into_global_context(
            'remove the label: ' + label_to_be_deleted + ' in file: ' + filename + ' at ' + str(self.index)
            + ' for plotting board :'
            + str(self.plottingBoardReference.get_plotting_index()) + ' in page: '
            + str(self.plottingBoardReference.get_page_index()))
    ...
    def verify_iso_cutting_information(self, string_in_cut_cmdline):
        ...
        elif iso_cut_information.__contains__("[") and iso_cut_information.__contains__(
                "]") and iso_cut_information.__contains__("/"):
            del self.subListISO[:]
            del self.subListISOIndex[:]
            del self.isoRangeCheckList[:]
            del self.isoScaleList[:]
    ...
    def verify_siso_cutting_information(self, string_in_cut_cmdline):
        ...
        elif siso_cut_information.__contains__("[") and siso_cut_information.__contains__(
                "]") and siso_cut_information.__contains__("/"):
            del self.subListSISO[:]
            del self.subListSISOIndex[:]
            del self.sisoRangeCheckList[:]
            del self.sisoCutPoints[:]
        ...
        if parameter_type == 'iso':
            if display_mode == 'represent_by_marker':
                if plotting_context.get_iso_display_mode() == 0:
                    for i in range(len(self.isoScaleList) + 1):
                        if i == 0:
                            range_temp = "<" + str(self.isoScaleList[i]) + "," + self.markerList[i % self.colorNumber][
                                0]
                        elif i < len(self.isoScaleList):
                            range_temp = "[" + str(self.isoScaleList[i - 1]) + "," + str(
                                self.isoScaleList[i]) + "[ : " + \
                                         self.markerList[i % self.colorNumber][0]
                        else:
                ...
                if plotting_context.get_iso_display_mode() == 0:
                    for i in range(len(self.isoScaleList) + 1):
                        ...
                        checkbox.setStyleSheet(
                            "background-color:" + self.colorList[
                                i % self.colorNumber] + "; margin-left:6px; color: white")
                        self.isoRangeCheckList.append(checkbox)
                elif plotting_context.get_iso_display_mode() == 1:
                    for i in range(len(self.isoScaleList) + 1):
            ...
            if display_mode == 'represent_by_color':
                ...
                if plotting_context.get_siso_display_mode() == 0:
                    for i in range(len(self.sisoScaleList) + 1):
                        if i == 0:
                            range_temp = "<" + str(self.sisoScaleList[i]) + "," + self.markerList[i % self.colorNumber][
                                0]
                        elif i < len(self.sisoScaleList):
                            range_temp = "[" + str(self.sisoScaleList[i - 1]) + "," + str(
                                self.sisoScaleList[i]) + "[ : " + \
                                         self.markerList[i % self.colorNumber][0]
                        else:
                            range_temp = ">=" + str(self.sisoScaleList[i - 1]) + "," + \
                                         self.markerList[i % self.colorNumber][0]
            ...
            if display_mode == 'represent_by_marker':
                if plotting_context.get_iso_display_mode() == 0:
                    for i in range(len(self.isoScaleList) + 1):
                        ...
                        self.isoRangeCheckList[i].setText(QString.fromUtf8(range_temp))
                        self.isoRangeCheckList[i].setStyleSheet(
                            "background-color: white; margin-left:6px; color: black")
                elif plotting_context.get_iso_display_mode() == 1:
                    for i in range(len(self.isoScaleList) + 1):
                        range_temp = str(self.isoCutPoints[i]) + "," + self.markerList[i % self.colorNumber][0]
                        self.isoRangeCheckList[i].setText(QString.fromUtf8(range_temp))
                ...
                if plotting_context.get_iso_display_mode() == 0:
                    ...
                    for i in range(len(self.isoScaleList) + 1):
                        range_temp = str(self.isoCutPoints[i]) + "," + self.markerList[i % self.colorNumber][0]
                        self.isoRangeCheckList[i].setText(QString.fromUtf8(range_temp))
                        self.isoRangeCheckList[i].setStyleSheet("background-color:" + self.colorList[
                            i % self.colorNumber] + "; margin-left:6px; color: white")
            self.isoDisplayMode = display_mode
        elif parameter_type == 'siso':
            if self.siso_existed:
                if display_mode == 'represent_by_marker':
                    ...
                    if plotting_context.get_siso_display_mode() == 0:
                        ...
                        for i in range(len(self.sisoScaleList) + 1):
                            range_temp = str(self.sisoCutPoints[i]) + "," + self.markerList[i % self.colorNumber][0]
                            self.sisoRangeCheckList[i].setText(QString.fromUtf8(range_temp))
                            self.sisoRangeCheckList[i].setStyleSheet("background-color:" + self.colorList[
                                i % self.colorNumber] + "; margin-left:6px; color: white")
                self.sisoDisplayMode = display_mode
    ...
    def iso_commandline_return_pressed(self):
        ...
        if self.analyserMediator.get_filetype_handling_from_context() != "lom" and self.plot.ask_if_curve_from_csv_existed():
            self.analyserMediator.generate_an_action_into_global_context(
                'set iso cut information: (' + self.isoCutInformation + ') for plotting board :'
                + str(self.plottingBoardReference.get_plotting_index()) + ' in page: '
                + str(self.plottingBoardReference.get_page_index()))
    ...
    def siso_commandline_return_pressed(self):
        ...
        if self.analyserMediator.get_filetype_handling_from_context() != "lom":
            self.analyserMediator.generate_an_action_into_global_context(
                'set siso cut information: (' + self.sisoCutInformation + ') for plotting board :'
                + str(self.plottingBoardReference.get_plotting_index()) + ' in page: '
                + str(self.plottingBoardReference.get_page_index()))
        self.sisoScaleList = self.generate_scale_list("siso")
    ...
    def set_sub_lists_of_iso_information(self):
        ...
        loaded_csvfiles_validity = self.analyserMediator.get_loaded_csv_files_validity_from_context(page_index,
                                                                                                    plotting_index)
    ...
    def set_iso_validity_for_lom(self):
        ...
        loaded_lomfiles_validity = self.analyserMediator.get_loaded_lom_files_validity_from_context(page_index,
                                                                                                    plotting_index)
        plotting_context = self.analyserMediator.get_a_plotting_context(page_index, plotting_index)
    ...
    def iso_range_check_list_changed(self):
        ...
        self.analyserMediator.generate_an_action_into_global_context(
            'set iso check information: (' + iso_check_list_string + ') for plotting board :'
            + str(self.plottingBoardReference.get_plotting_index()) + ' in page: '
            + str(self.plottingBoardReference.get_page_index()))
    ...
    def iso_gradient(self, start_color, finish_color):
        ...
        if self.siso_existed:
            ...
            sub_list_iso_values = [[[] for y in range(len(self.isoRangeCheckList))] for y in
                                   range(len(loaded_csvfiles))]
            sub_list_iso_indexs = [[[] for y in range(len(self.isoRangeCheckList))] for y in
                                   range(len(loaded_csvfiles))]
            cpt = 0
    ...
    def siso_range_check_list_changed(self):
        ...
        if self.lom_existed:
            ...
            for lom_file_name in loaded_lomfiles:
                if not loaded_lomfiles_validity[loaded_lomfiles.index(lom_file_name)]:
                    continue
                for label_name in plotting_context.get_label_list_from_lom_file(lom_file_name):
                    siso_validity = plotting_context.get_siso_parameter_validity_from_lom_file(label_name,
                                                                                               lom_file_name)
                    del self.siso_check_list[:]
                    for i in range(len(self.sisoRangeCheckList)):
                        if self.sisoRangeCheckList[i].isChecked():
        ...
        self.analyserMediator.generate_an_action_into_global_context(
            'set siso check information: (' + siso_check_list_string + ') for plotting board :'
            + str(self.plottingBoardReference.get_plotting_index()) + ' in page: '
            + str(self.plottingBoardReference.get_page_index()))
